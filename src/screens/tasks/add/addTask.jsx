import {
  ...
  import FormInputs from "../../../json/forms/Task";
  ...
  export const AddTask = (props) => {
    ...
    const breadcrumb = {
      title: "Tarea",
      subtitle: task?.id ? "Modificar" : "Agregar",
      right_content: null,
    };
    ...
    const setInitialData = async () => {
      ...
      FormInputs["structure"] = task ? task : taskStructure;
    ...
    const onSuccessSave = () => {
      let taskID = dataForm[FormInputs.form_name]?.structure?.id;
      setSaving(false);
      LocalStorage.set(Constants.LOCALSTORAGE.UPDATED, "tasks");
      showMessage({
        message: Texts.success.title,
        description: taskID
          ? Texts.success.task.update
          : Texts.success.task.create,
        duration: 3000,
        type: "success",
      });
  
      if (!taskID) setDataForm({ [FormInputs.form_name]: FormInputs });
    };
