...
export const CustomTextInput = ({data, formName}) => {
  ...
  const getBorderColor = () => {
    return !Validations.checkRequiredField(data, dataForm[formName], checkRequired[formName]).status
        ...
        <TextInput
          ...
          onChangeText={text => {
            setDataForm({
              [formName]: {
                ...dataForm[formName],
                structure: {...dataForm[formName].structure, [data.name]: text},
              },
            });
          }}
        />
      </View>
      {!Validations.checkRequiredField(data, dataForm[formName], checkRequired[formName]).status ? (
        <Text style={Style.text_red}>{Validations.checkRequiredField(data, dataForm[formName], checkRequired[formName]).text}</Text>
      ) : null}
    </View>
  );
};
