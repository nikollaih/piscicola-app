...
export const get = async (token: String, filter: String = "") => {
  let response = await fetch(Constants.API.URL + "fishes/list/" + filter, {
    headers: {
...
export const create = async (token: String, data: any) => {
  let response = await fetch(Constants.API.URL + "fishes", {
    headers: {
      ...Constants.CONFIG.HEADERS,
      Authorization: "Bearer " + token,
    },
...
export const remove = async (token: String, fishID: Number) => {
  let response = await fetch(Constants.API.URL + "fishes/" + fishID , {
    headers: {
      ...Constants.CONFIG.HEADERS,
      Authorization: "Bearer " + token,
    },
