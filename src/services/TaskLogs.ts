...
export const get = async (token: String, puID: String = "") => {
  let response = await fetch(`${Constants.API.URL}tasks/logs/list_by_productive_unit/${puID}`, {
    headers: {
...
export const create = async (token: String, data: any) => {
  ...
  let response = await fetch(`${Constants.API.URL}tasks/logs`, {
    headers: {
      ...Constants.CONFIG.HEADERS,
      Authorization: "Bearer " + token,
    },
...
export const remove = async (token: String, taskLogID: Number) => {
  let response = await fetch(`${Constants.API.URL}tasks/logs/${taskLogID}` , {
    headers: {
      ...Constants.CONFIG.HEADERS,
      Authorization: "Bearer " + token,
    },
